...
from Nodeeditor.Edge.GraphicalEdge import DrawGraphicalEdge
from Nodeeditor.Edge.EdgeDraggingFunc import AllEdgeDraggingFunctions
from Nodeeditor.Edge.EdgeReroutingFunc import AllEdgeReroutingFunctions
from Nodeeditor.Edge.EdgeIntersectFunc import AllEdgeIntersectFunctions
from Nodeeditor.Edge.EdgeSnappingFunc import AllEdgeSnappingFunctions
from Nodeeditor.Edge.GraphicalCutlineEdges import DrawGraphicalCutLine
...
class DrawGraphicalView(QGraphicsView):
    ...
    def __init__(self, grScene: 'QDMGraphicsScene', parent: 'QWidget' = None):
        ...
        :type grScene: :class:`~nodeeditor.GraphicalScene.DrawGraphicalScene`
        ...
        # edge dragging
        self.dragging = AllEdgeDraggingFunctions(self)

        # edges re-routing
        self.rerouting = AllEdgeReroutingFunctions(self)

        # drop a node on an existing edge
        self.edgeIntersect = AllEdgeIntersectFunctions(self)

        # edge snapping
        self.snapping = AllEdgeSnappingFunctions(self, snapping_radius=EDGE_SNAPPING_RADIUS)

        # cutline
        self.cutline = DrawGraphicalCutLine()
    ...
    def middleMouseButtonPress(self, event: QMouseEvent):
        ...
        if DEBUG_MMB_SCENE_ITEMS:
            if isinstance(item, DrawGraphicalEdge):
    ...
    def leftMouseButtonPress(self, event: QMouseEvent):
        ...
        # logic
        if hasattr(item, "node") or isinstance(item, DrawGraphicalEdge) or item is None:
    ...
    def leftMouseButtonRelease(self, event: QMouseEvent):
        ...
        try:
            ...
            if self.mode == MODE_EDGES_REROUTING:
                ...
                self.rerouting.stopRerouting(item.socket if isinstance(item, DrawGraphicalSocket) else None)
